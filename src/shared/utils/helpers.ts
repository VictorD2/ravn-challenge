...
export const mergeQuerysParams = (...classes: Array<string | undefined>) => {
  return classes
    .filter((item) => typeof item !== undefined)
    .filter(Boolean)
    .join(", ");
...
export const getRandomAvatar = () => {
  ...
  return `https://picsum.photos/${randomNumber}/${randomNumber}`;
};

export const wordsToNum = (word: string) => {
  if ("EIGHT" === word) return 8;
  if ("FOUR" === word) return 4;
  if ("TWO" === word) return 2;
